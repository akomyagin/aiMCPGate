...
события из вывода исключаются (`--upstream` работает для обоих типов).
...
сбрасывается при завершении шлюза.
...
раньше такой ключ молча игнорировался (до v0.5.0). Конкретная выгода: опечатка в
